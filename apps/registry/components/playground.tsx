...
function EditorArea() {
  ...
  return (
    <div className="relative h-[540px] overflow-hidden rounded-md border-b">
      <div
        className={cn(
      ...
      {loading && (
        <GradientLoader className="absolute inset-0 z-10 border bg-background" />
      )}
    </div>
  )
}
  ...
  useEffect(() => {
    const post = () => {
      containerRef.current
        ...
        .forEach((f) =>
          f.contentWindow?.postMessage({ __sapaTheme: true, dark: isDark }, "*")
        )
    }
    post()
        ...
        <div className="mb-6 inline-flex rounded-md border bg-muted/50 p-0.5 text-xs">
          {FRAMEWORKS.map((f) => (
            ...
            >
              {f.label}
              {f.disabled && (
                <span className="rounded-sm bg-linear-to-br from-sapa-warning to-sapa-error px-1 py-0.5 text-[9px] font-semibold uppercase leading-none text-white">
                  Soon
                </span>
              )}
            </button>
